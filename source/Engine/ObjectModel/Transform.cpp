...
void Transform::Rotate(const glm::vec3& axis, float angleRadians) {
    glm::quat delta = glm::angleAxis(angleRadians, glm::normalize(axis));
    m_LocalRotation = delta * m_LocalRotation; // apply delta before current
    SetRotationDirty();
}

void Transform::Rotate(const glm::vec3& eulerAngles) {
    glm::quat delta = glm::quat(eulerAngles); // expects radians
